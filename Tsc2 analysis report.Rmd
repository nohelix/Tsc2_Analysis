...
github = "Z:/"
setwd(github)
source(paste0(github, "GitHub/Tsc2_Analysis/Rxn & TH analysis.R"))
...
kruskal.test(Rxn ~ `Dur (ms)`, 
             data = Rxn_overall_by_Duration %>% 
                    filter(Duration == "Alone" & Intensity %in% c(15, 20, 25, 30, 35, 40, 45, 50, 60, 70, 80)))

postHoc <- 
  FSA::dunnTest(Rxn ~ interaction(Duration, `Dur (ms)`),
           data = Rxn_overall_by_Duration %>% filter(Intensity %in% c(15, 20, 25, 30, 35, 40, 45, 50, 60, 70, 80)),
           method = "bonf")
...
postHoc <- 
  FSA::dunnTest(Rxn ~ interaction(Genotype, Duration, `Dur (ms)`),
           data = Rxn_overall_by_Duration %>% filter(!(ID %in% partial.data) & Intensity %in% c(15, 20, 25, 30, 35, 40, 45, 50, 60, 70, 80)),
           method = "bonf")
...
```{r}
save(df, Master_summary, loaded_files, Data_over_TH, Rxn, Rxn_overall_by_Duration, file = "Tsc2_analysis.Rdata")
```
